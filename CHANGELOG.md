...
### 新增
- **本地模型管理**：在 WebUI 中下载、启用和管理 CLIP / 人脸识别模型。
- **语义搜索**：用自然语言描述图片内容，例如“海边日落”，即可搜索媒体库图片。
- **人脸页面**：自动识别人脸并按人物聚合，支持查看、重命名、合并、拆分。
- **人脸封面选择**：可手动选择更好看的人脸作为人物封面。
- **TOTP 双因素登录**：支持绑定验证码应用，为 WebUI 登录增加二次验证。

### 优化
- **模型运行进度**：下载、依赖安装、CLIP 扫描和人脸扫描都有统一进度提示。
- **人脸入库质量控制**：可调整清晰度、大小、置信度阈值，减少模糊或过小的人脸。
- **面孔页面 UI**：人物卡片和详情抽屉更清晰，批量选择与封面操作更直观。
